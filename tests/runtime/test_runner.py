...
class RunnerTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_pause_resume(self):
        ...
        final = await self.runner.resume_run(paused.run_id, approval_id=paused.approval["id"], approved=True)
        self.assertEqual(final.status, "completed")
        self.assertEqual(final.state["data"]["task_status"], "closed")
        events = self.runner.get_events(paused.run_id)
        self.assertEqual([e["seq"] for e in events], list(range(1, len(events) + 1)))
        self.assertEqual(sum(e["type"] == "approval_required" for e in events), 1)
        self.assertEqual(sum(e["type"] == "run_started" for e in events), 1)
        self.assertEqual(self.runner.get_events(paused.run_id, after_seq=events[-1]["seq"]), [])

    async def test_reject(self):
        paused = await self.start()
        final = await self.runner.resume_run(paused.run_id, approval_id=paused.approval["id"], approved=False)
    ...
    async def test_stale_approval(self):
        ...
        with self.assertRaises(RunConflictError):
            await self.runner.resume_run("run-1", approval_id="wrong", approved=True)
        self.assertEqual(self.runner.get_run("run-1").status, "waiting_approval")

    async def test_concurrent_resume_only_applies_once(self):
        paused = await self.start()
        async def resume():
            return await self.runner.resume_run("run-1", approval_id=paused.approval["id"], approved=True)
    ...
    async def test_snapshot_is_detached(self):
        ...
        snapshot.approval["context"]["changes"]["torque"] = 99
    ...
    async def test_failure_is_projected(self):
        class Broken:
            async def invoke(self, *args):
                return ToolResult("fatal_error", error="offline")
        ...
        self.assertEqual(sum(e["type"] == "run_failed" for e in self.runner.get_events("run-1")), 1)
        with self.assertRaises(RunConflictError):
            await self.runner.resume_run("run-1", approval_id="anything", approved=True)
    ...
    async def test_invalid_resume_does_not_consume_approval(self):
        ...
        with self.assertRaises(ValueError):
            await self.runner.resume_run("run-1", approval_id=paused.approval["id"], approved="false")
        self.assertEqual(self.runner.get_run("run-1").status, "waiting_approval")
